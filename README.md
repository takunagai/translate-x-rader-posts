...
1. **起動制御**（`content.js`）: X は SPA なので、`history.pushState`/`replaceState` のラップ + `popstate` + 500ms の URL ポーリングで遷移を検知し、URL が `/i/radar/` を含むときだけ翻訳エンジンを起動。
...
- **`world: "MAIN"` の採用理由**: 組み込み翻訳 API への確実なアクセスと、X 本体の SPA 遷移（`history.pushState`）の確実な捕捉のため。代償として **`chrome.*` API（`chrome.storage` 等）は使えない**ので、設定 UI・永続化を足す場合は isolated world ブリッジの追加か world 変更が前提になる。
- **二重注入ガード**: 拡張リロード等での再注入に備え、`ns.__contentLoaded` で `history` の多重ラップ・タイマー重複を防止。
